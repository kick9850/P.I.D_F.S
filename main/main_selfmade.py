...
image_datasets = {x: datasets.ImageFolder(os.path2data.join(data_dir, x),
                                          data_transforms[x])
                  for x in ['newtrain', 'newtest', 'test']}
...
def visualize_model(model, num_images=6):
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(test_ds):
...
params_train = {
    ...
    'path2weights':'./models/slf_plantdoc.pt'
}
